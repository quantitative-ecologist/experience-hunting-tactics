...
data[, ":=" (Zspeed_novice              = ifelse(cumul_xp_pred_bins == "novice", Zspeed_group, NA),
             ...
             hunting_success_novice     = ifelse(cumul_xp_pred_bins == "novice", hunting_success, NA),
             ...
             hunting_success_interm     = ifelse(cumul_xp_pred_bins == "intermediate", hunting_success, NA),
             ...
             hunting_success_advanced   = ifelse(cumul_xp_pred_bins == "advanced", hunting_success, NA))]
...
# Add observation-level random effects by xp group

data[, ":=" (obs1 = ifelse(cumul_xp_pred_bins == "novice", 
                           1:nrow(data[cumul_xp_pred_bins == "novice"]),
                           NA),
             obs2 = ifelse(cumul_xp_pred_bins == "intermediate", 
                           1:nrow(data[cumul_xp_pred_bins == "intermediate"]),
                           NA),
             obs3 = ifelse(cumul_xp_pred_bins == "advanced", 
                           1:nrow(data[cumul_xp_pred_bins == "advanced"]),
                           NA))
    ]

# Model formulas
hunting_success_novice <-   bf(hunting_success_novice | trials(4) + subset(sub1) ~
                                Zgame_duration + 
                               (1 |a| player_encode_id) +
                               (1 | obs1)) +
                            binomial()

hunting_success_interm <-   bf(hunting_success_interm | trials(4) + subset(sub2) ~
                                Zgame_duration +
                                (1 |b| player_encode_id) +
                                (1 | obs2)) +
                            binomial()

hunting_success_advanced <- bf(hunting_success_advanced | trials(4) + subset(sub3) ~
                                Zgame_duration +
                                (1 |c| player_encode_id) +
                                (1 | obs3)) +
                            binomial()
...
priors <- c(
  ...
  set_prior("normal(0, 5)",
            class = "b",
            resp = c("huntingsuccessnovice", "huntingsuccessinterm", "huntingsuccessadvanced")),
  set_prior("lkj(2)", 
            class = "cor",
...
mv_model <- brm(speed_novice +
              ...
              #  ambush_advanced +
                ...
                prey_speed_advanced +
                hunting_success_novice +
                hunting_success_interm +
                hunting_success_advanced +
                set_rescor(FALSE),
              warmup = 300, 
              iter = 10300,
              thin = 8,
