...
*expliquer avant que j'ai fait 2 modèles et que j'ai comparé par LOO afin de déterminer si la relation entre le succès et l'expérience variait d'un joueur à l'autre. Dire que j'ai suivi le workflow the Perdersen, Wood, etc.*

We estimated the relationship between hunting success and experience using a Bayesian generalized additive mixed model with thin plate regression splines. Because a maximum of four prey can be captured, we used the beta-binomial family distribution to estimate the probability of capturing all prey while controlling for overdispersion. The model parameters were estimated with a logit link function with the number of knots set to ten, while the dispersion parameter ($\phi$) was estimated with an identity link. The model had the following form :

\[
\begin{align}
logit (P_{i}) &= (\beta_{0} + id_{0,i}) \\
&\text{ } + \text{ } \beta_{1} \textit{time}_{i} \\
&\text{ } + \text{ } f(\textit{cumul. xp}_{i}) \\
\end{align}
\tag{1}
\]

where $P_{i}$ is drawn from a Beta distribution ($P_{i} ~ Beta(a_{i}, b_{i})$) with shape ($a_{i} = \frac{P_{i}}{\phi}$) and scale ($b_{i} = \frac{1 - P_{i}}{\phi}$) parameters. Here, $\beta_{0}$ is the general intercept estimating the mean hunting success, and $id_{0,i}$ is the random intercepts term for the the predator player ID, for which we assumed followed a Gaussian distribution with estimated variance ($id_{0,i} ~ N(0, V_{id})$). We controlled for the match duration
