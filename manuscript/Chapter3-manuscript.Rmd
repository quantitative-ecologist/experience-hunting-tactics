...
All our statistical analyses were conducted on Cedar (https://docs.alliancecan.ca/wiki/Cedar), a computer cluster provided by the Digital Research Alliance of Canada. The operating system for Cedar is CentOS Linux 7. The models were fitted in R (version 4.1.2) using Markov chain Monte Carlo sampling with the package "brms" version 2.16.3 [@Burkner2017], and "cmdstanr" version 0.4.0 [@Gabry2021] as the backend for estimation (cmdstan installation version 2.28.2).

### Effect of experience on hunting success

We estimated the relationship between hunting success and experience using a Bayesian generalized additive mixed model with thin plate regression splines. Because a maximum of four prey can be captured, we used the beta-binomial family distribution to estimate the success of predators while controlling for overdispersion. The model parameters were estimated with a logit link function, while the dispersion parameter ($\phi$) was estimated with an identity link. The model had the following form :

\[
\begin{align}
logit (P_{ij}) &= (\beta_{0} + id_{0,i} + \phi_{0,ij}) \\
...
The number of knots was set to 10.
