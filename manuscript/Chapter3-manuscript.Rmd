...
dat <- fread(file.path(path, "FraserFrancoetalXXXX-data.csv"),
             select = c("match_encode_id", "game_mode",
...
dat <- unique(dat)
dat[, c(1:6) := lapply(.SD, function (x) as.factor(x)), .SDcols = c(1:6)]
...
We were provided with raw datasets from the videogame company that spanned a period of 6 months of matches recorded for each player. 

# (mean = `r toString(format(round(mean(data$space_covered_rate, na.rm = TRUE), digits = 2) (pour écrire dans le texte)
