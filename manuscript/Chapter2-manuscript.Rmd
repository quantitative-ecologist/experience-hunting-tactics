---
title: "Predator-prey interactions remain stable across a predator's experience in an online videogame"
output: 
  word_document:
...
where $j$ is the index of the experience levels. The model is fitted on the three experience levels denoted by $T_{j}$, where $j$ ranges from 1 to 3 (i.e. novice, intermediate, advanced). The $d$ subscript indicates that a term is from the dispersion part of the model. The $b$ terms are vectors of fixed effects with their design matrices $X$. The terms $en$, $av$, and $id$ are the vectors of random environment, random avatar, and random individual identity effects associated with their design matrices $M$, $N$, and $Z$. For all experience levels of the predator and prey speed, we assumed that the game environment and avatar random effects followed a Gaussian distribution with estimated standard deviation ($en \sim N(0,\ I\ \sigma_{en})$ and $av \sim N(0,\ I\ \sigma_{av})$). $I$ is the identity matrix, with $I\sigma$ indicating that the random effects are independently and identically distributed. All the individual identity random effects (mean and dispersion) across all response variables and experience levels are indexed in vector $u$. The $u$ vector follows a multivariate Gaussian distribution $u \sim MVN(0,\ \Sigma)$ where $\Sigma$ is a 15x15 (co)variance matrix of the individual identity standard deviations $\sigma_{id_i}$ across all experience levels and response variables.
...
Learning from experience is predicted to increase individual predator foraging specialization when prey distribution, behaviour, and movement is predictable through time [@Estes.etal2003;@Tinker.etal2008;@Tinker.etal2009]. However, if predators encounter varying prey with a greater behavioural range through time, then learning should increase behavioural flexibility [@Stephens1993;@Ishii.Shimada2010;@Mery.Burns2010;@Snell-Rood2013;@Wright.etal2022]. We found that predators in *DBD* increased their hunting success with experience substantially, suggesting that learning through repeated interactions played a role in the development of predator expertise. While there was no directional increase in either behavioural specialization or flexibility with experience at the population level, individual predators developed their own behavioural trajectories over successive encounters with the prey. This was characterized by a continuum of hunting styles ranging from a slower and flexible strategy to a highly specialized fast-paced strategy, with some predators switching between the two with experience. The behavioural strategies of predators and prey were matched across experience, as both were probably adjusting to each other with specialized cursorial hunters encountering similar groups of prey while slower and flexible hunters encountered varying groups of prey. Overall, predators along the flexible-specialist continuum achieved similar success, although flexible hunters were slightly less successful as advanced hunters.


## The development of expertise with experience

Our results suggest that expertise is honed through extensive practice, but also depends on an individual's capacity to adjust to difficulty. The predator population increased and stabilized its success with experience in a diminishing returns fashion as it is typically found in empirical studies of expertise [reviewed in @Dukas2019]. The prey's speed was important in mediating this pattern as well as the observed differences in expertise acquisition among predators, probably because the prey increased their speed as predators gained experience. We previously showed in *DBD* that faster movement is an effective strategy used by the prey to avoid predation [@FraserFranco.etal2022], and other studies have found that as well [@Walker.etal2005;@Kelley.Magurran2011;@Martin.etal2022]. Hunting faster prey requires costly and specialized cognitive abilities and coordination in predators [@Kelley.Magurran2011], and that could explain why they are more difficult to capture. Thus, predators that couldn't properly hunt at high speeds or develop counterstrategies for faster prey were at a disadvantage.

However, even after we took prey speed into account, there were still important differences in expertise acquisition among predators, suggesting that other antipredator tactics were potentially involved. For example, if the prey used camouflage or hiding to avoid being detected by the predator [Bowen.etal2002;@Carey.Wahl2011;@Nomura.etal2011], then predators may only find them if they learned how to exploit the visual cues that facilitate cryptic prey detection, such as habitat characteristics or prey colour patterns [@Ehlinger1989;@Hughes.etal1992;@Warburton2003;@Szopa-Comley.etal2020]. In addition, longer time delays between hunting events may exacerbate slower learning or even forgetting [@Endler1991;@Wright.etal2022]. In *DBD*, a predator that played 300 matches in the span of six months might forget more critical information related to prey detection or escape patterns than one that played 300 matches in the span of six days.


## Predator-prey interactions across experience

The predator population maintained their speed and degree of specialization throughout the study period, and all individuals used a similar tactic (i.e. mean speed). However, differences in specialization were important and increased slightly with experience. Predators that specialized in hunting at high speeds encountered groups of prey with similar speeds across matches, while those that adopted a slower and flexible strategy encountered varying groups of prey. This pattern might arise because fast-paced hunting is a specialized tactic suited for prey that use rapid evasive movements [@Endler1991;@Bro-Jorgensen2013;@Wilson.etal2018], while a slower and flexible tactic can be useful to minimize the consequences of uncertainty when prey escape unpredictably [@Szopa-Comley.Ioannou2022]. However, it is more plausible that this reflects reciprocal adjustments in speed by predators and prey as they interacted [@Kishida.etal2006;@Kishida.etal2009;@Edgell.Rochette2009a;@McGhee.etal2013a], because prey can also learn how to avoid predation [REFS]. 

Surprisingly, although predators changed their specialization across experience, the behavioural interactions between the predator and the prey remained generally stable. This may in part be due to almost half (44%) of the population showing close to no change in specialization with experience. If these predators were already successful as novices, then there would be no point to change their tactic with experience. However, 66% of the predator population did alter their behaviour with experience to different degrees. Thus, if the prey also learned through repeated interactions with the predators, then it is possible that it also contributed in stabilizing the system, similar to Red Queen dynamics [@brockhurst2014].

The coupling of blabla...pour commencer le paragraphe (moins mettre l'accent sur le changement mais sur le type de relation et ce qui la drive)

When predators were novices, our results align with a matching-response relationship [@Mougi.Kishida2009], because faster-specialist and slower-flexible hunters preyed upon different prey, but they both overall achieved similar success. As predators became advanced, slower-flexible hunters became marginally less successfull, suggesting that faster-specialist hunters were probably learning better how to hunt since faster prey were also more difficult to capture. *ici ajouter que le long term de ça n'est pas certain. il faut que je lise de la littérature explicant pourquoi ça peut être moins bon d'être généraliste*

These types of reciprocal responses to interactions have been observed in other predator-prey systems [@Kishida.etal2006;@Kishida.etal2009;@Edgell.Rochette2009a;@McGhee.etal2013a], but their broader consequences remain unclear.

flexible hunters were slightly less successful than specialists. *expliquer pourquoi les généralistes ont moins bien performé ici*
*Modifier cette phrase pour appuyer que les généralistes étaient moins bons à long terme: It remains uncertain whether this phenotypic matching enables predators with different degrees of specialization to achieve equal success within populations [@Woo.etal2008].*

*Moreover, even though the relationship was weak, there was an important change in the correlation between the predator's IIV and the mean speed of the prey they encountered.* *pas sûr que je vais ajouter ça*


*ici c'est mon bout sur le matching response je crois...*
When prey express defensive phenotypes such as high speeds, selection on predators should be strong because of the consequences of repeated failures. If predators improve their exploitative ability, then the consequences of hunting these prey should decrease [@BrodieIII.BrodieJr.1999]. Similarly, theoretical studies predict that learning may attenuate selection [@Anderson1995;@Ancel1999;@Borenstein.etal2006;@Paenke.etal2007], but failing to account for reciprocal interactions may mask more complex outcomes. Here, learning appears to have reduced differences in success among predators, but success remained lower when predators hunted faster prey. Again, if prey learned how to avoid the predators, then this would explain why advanced hunters still struggled to achieve similar success against faster prey compared to slower prey. Thus, disentanling the consequences of learning from reciprocal interactions may reveal important insights on predator-prey interactions. 


## Conclusions

A general assumption of predator-prey studies is that predators maximize success by matching their tactic to their prey [@abramsEvolutionPredatorPreyInteractions2000], yet, it has remained largely unknown whether this results from predators learning how to hunt their prey, in part because of the challenges of investigating direct interactions in the wild. By capitalizing on a virtual predator-prey system where interactions were directly monitored, we found *support of our prediction that...*

*Thus, our results provide direct evidence that predators matching their tactic to their prey may be key in maintaining behavioural diversification in predator-prey systems.*

that individual predators displayed large differences in the development of their expertise. With experience, the population became increasingly distinct along a flexible-specialist continuum, while interactions remained largely stable. Overall, all hunters achieved similar hunting success.

Virtual systems are increasingly recognized among ecologists as being useful to test hypotheses on consumer-resource interactions [@Barbe.etal2020;@Beauchamp2020;@Cere.etal2021;@FraserFranco.etal2022], but also in other fields such as citizen science and conservation [@Sandbrook.etal2015;@Redpath.etal2018;@Duthie.etal2021]. We therefore hope that our study will inspire more collaborations between scientists and the videogame industry to tackle fundamental questions in ecology.
