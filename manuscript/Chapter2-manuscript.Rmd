...
Keywords: foraging behaviour, individual specialization, learning, behavioural plasticity, prey predictability, online videogames
...
# Extract the correlations to report values in the text
...
# =======================================================================
 ...
 # Model
 path <- here::here("outputs", "02_outputs_models")
...
# Reshape the table -----------------------------------------------------
 ...
 # Long format
 table <- melt(draws,
               measure = patterns("^cor_predator_id"),
 ...
 cor_tab <- unique(table[, c(1, 3:5)])

 # Round values to 3 digits
 cor_tab[, c(2:4) := lapply(.SD, function (x) {round(x, digits = 3)}),
         .SDcols = c(2:4)]
...
In Reimche et al. 2020 : in the discussion we can read that we still don't understand the factors driving phenotypic (mis)matching. In my case, it appears that learning may play a role.
Revoir mes résultats dans le cadre des résultats de Glaudas et al. 2019. Les 2 concordent bien.
Voir Miszei comme exemple parmi d'autres pour trait matching.
check Mougi etal. 2011 for arms-races based on phenotypic plasticity!
